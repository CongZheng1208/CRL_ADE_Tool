      ...
      <n-card title="设备选择" :bordered="false" class="h-full rounded-16px shadow-sm">
				<div class="select-container">
					...
					<!-- <div class="select-item">
						<div class="device-number">
							...
							<div class="buttons">
								<n-button
									v-for="i in 6"
									:key="i"
									:type="appStore.controll.deviceId === i ? 'primary' : 'default'"
									@click="appStore.controll.deviceId = i"
								>
								{{ i }}
								</n-button>
							</div>
						...
						<div class="device-number">
							<div class="label">POS编号</div>
							<div class="buttons">
								<n-button
									v-for="i in 9"
									:key="i"
									:type="appStore.controll.pos === i ? 'primary' : 'default'"
									@click="appStore.controll.pos = i"
								>
									{{ i }}
								</n-button>
							</div>
					...
					<div class="select-item">
						<div class="device-number">
							...
							<div class="mode-switch">
								<n-switch v-model:value="appStore.controll.isOnAir" size="large">
								...
								<div class="myText">{{ appStore.controll.isOnAir ? '空中' : '地面' }}</div>
							</div>
						</div>

						<div class="device-number">
							<div class="label">设备方向</div>
							<div class="mode-switch">
								<n-switch v-model:value="appStore.controll.isRight" size="large">
									<template #checked-icon>
										<svg-icon :icon="'fa-regular:arrow-alt-circle-right'" class="text-30px" />
									</template>
									<template #unchecked-icon>
										<svg-icon :icon="'fa-regular:arrow-alt-circle-left'" class="text-30px" />
									</template>
								</n-switch>
								<div class="myText">{{ appStore.controll.isRight ?  '右侧' : '左侧' }}</div>
					...
					<div class="select-item">
						<div class="device-number">
							<div class="label">当前选择设备：</div>
							<icon-local-service-error class="text-150px sm:text-120px text-primary" />
						</div>
				  </div> -->
			  </div>
      </n-card>
  ...
  setup() {
    ...
    function handleSubmit() {
      ...
      console.log('上传文件：', files.value)
			alert('上传成功！')
			axios.post(`${appStore.userinfo.remoteUrl}/controll`, {
