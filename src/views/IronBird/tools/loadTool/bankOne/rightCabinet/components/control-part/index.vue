					...
					<div class="mode-switch">
						<n-switch v-model:value="currentPage" size="large">
							<template #checked-icon>
								<svg-icon :icon="'fluent-mdl2:open-file'" class="text-30px" />
							</template>
							<template #unchecked-icon>
								<svg-icon :icon="'fluent-mdl2:document-search'" class="text-30px" />
							</template>
						</n-switch>
  ...
  setup() {
    ...
    function handleSubmit() {
      ...
      console.log('上传文件：', files.value)
			console.log(`${appStore.userinfo.remoteUrl}`)
    ...
    function removeFile(index: number) {
      files.value.splice(index, 1)
		...
		function createLabel (level: number): string {
			if (level === 4) return '一级目录'
			if (level === 3) return '二级目录'
			if (level === 2) return '三级目录'
			if (level === 1) return '四级目录'
			return ''
		}
